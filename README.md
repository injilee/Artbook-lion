...
-  <code>cs 지식:</code> 피가 되고 살이 된다.
-  <code>redux</code>를 처음 써보면서 느낀 점
   -  state를 한 곳에서 관리하면 편하다는 걸 알고 있었지만 직접 겪어보니 너무 편하긴 했다! redux부터 공부하다가 redux-toolkit으로 프로젝트에 적용해보니 Boilerplate code가 향상된 걸 보다보니 코드의 효율성이 중요하고 가독성 또한 중요하다는 걸 다시 깨달았다.
   -  예전에는 새로운 라이브러리를 이해해서 활용하는데 시간이 좀 걸렸었는데 점차 시간이 줄어드는게 느껴졌다. 공부는 꾸준히 해야 한다는 것!
-  프로젝트 규모가 커질수록 단순하고 정교해야 한다.
   -  그동안 만들었던 프로젝트 중에 '그림책 라이언'이 꽤 많은 기능이 들어간 프로젝트였다. 대부분 간단한 기능들 몇 뿐이었다. 간단한 기능이지만 동작을 바로 이해하기 어려웠던 기능도 있었고 만들어야 하는 페이지 수가 늘어나면 어떻게 효율적으로 코드를 짜야하는가에 대한 고민도 많아졌다. 좋은 코드를 많이 보면서 좋은 기술과 노하우들을 훔쳐와야겠다(?)
   -  개발 블로그에 내가 겪은 삽질들을 많이 남겨야겠다. 삽질은 멋진 실수들을 증명할 수 있는 것이라 생각한다.
